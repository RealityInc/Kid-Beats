...
export function analyzeSignal(data, sampleRate){
  ...
  return {durationSec:Number(durationSec.toFixed(2)),analysisSource,bpm,bpmCandidates,onsets:onsets.map(v=>Number(v.toFixed(3))),interOnsetIntervals:ioi.map(v=>Number(v.toFixed(3))),key:top.confidence<0.45?'uncertain':top.key,scale:top.confidence<0.45?'uncertain':top.scale,keyCandidates:keyCandidates.slice(0,3),scaleCandidates:scaleCandidates.slice(0,3),scaleFit:{fitCount:top.fitCount,totalNotes:notes.length},pitchRange,pitchContour,notes,rhythm:{onsets:onsets.map(v=>Number(v.toFixed(3))),gridFit:bpmCandidates[0]?.confidence||0,confidence:rhythmConfidence,inferredFrom:[onsets.length?'amplitude':null,notes.length?'pitch changes':null].filter(Boolean)},mood:{label:'estimated',method:'estimated from tempo, pitch range, loudness, and mode',factors:{tempo:bpm,pitchRange,loudness:Number(avgEnergy.toFixed(3)),mode:top.scale}},uncertain:top.confidence<0.45||notes.length<3?'not enough signal':null};
}
